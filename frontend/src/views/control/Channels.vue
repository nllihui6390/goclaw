...
async function toggleChannel(channel, val) {
  channel.config.enabled = val
  try {
    await api.updateChannel(channel.key, channel.config)
...
async function saveChannel() {
  ...
  try {
    ...
    // 同步到内存
    Object.assign(editChannel.value.config, editConfig.value)
      ...
      >
        <div class="card-inner" @click="openEdit(channel)">
          ...
          <div class="card-bottom">
            ...
            <el-switch
              :model-value="channel.config.enabled"
